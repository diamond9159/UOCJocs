...
void purchaseTableInit(tPurchaseTable* purchasesTable) { purchasesTable->nPurchases = 0; }
...
void getPurchaseObject(const char* str, tPurchase* purchase)
{
    ...
    /* read purchase data */
    sscanf(str, "%d %d %s %d %d/%d/%d %f", &auxPurchaseId, &auxStoreId, buffer, &auxNPurchased, &purchase->date.day,
           &purchase->date.month, &purchase->date.year, &auxPrice);
    ...
    if(tabPurchase->nPurchases == MAX_ORDERS) {
        *retVal = ERR_MEMORY;
        ...
        if(iPurchase == NO_PURCHASE) { /* Purchase not existing */
            ...
            orderCpy(&(tabPurchase->table[iPurchase].orders[tabPurchase->table[iPurchase].nOrders]),
                     newPurchase.orders[0]);
...
{
    /* Build the string */
    snprintf(str, maxSize - 1, "%d %d %s %d %d/%d/%d %.2f", (int)purchase.purchaseId, (int)purchase.storeId,
             order.bgame, order.nPurchased, purchase.date.day, purchase.date.month, purchase.date.year, order.price);
}

void purchaseTableFilterByYearAndStore(tPurchaseTable* tabPurchases, int year, tStoreId storeId, tError* retVal,
                                       float* benefit)
{
    /*****************************************/
    /* Exercise 8b */
    ...
    // Initialize the benefit and retVal
    *benefit = 0;
    *retVal = OK;

    // Loop through each purchase in the table
    for(int i = 0; i < tabPurchases->nPurchases; i++) {

        // Check if the purchase was made in the store during the specified year
        if(tabPurchases->table[i].storeId == storeId && tabPurchases->table[i].date.year == year) {

            // Add up the price of all orders in the purchase
            for(int j = 0; j < tabPurchases->table[i].nOrders; j++) {
                *benefit += tabPurchases->table[i].orders[j].nPurchased * tabPurchases->table[i].orders[j].price;
            }
        }
    }

    // If no purchases were found, set retVal to ERROR
    if(*benefit == 0) {
        *retVal = ERROR;
    }
...
void lessPopularCategory(tPurchaseTable* tabPurchases, tStoresTable* tabStores, tStoreId id, tError* retVal,
                         tGameCategory* cat)
{
    /*****************************************/
    /* Exercise 9b */
