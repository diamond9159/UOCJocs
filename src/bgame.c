...
void bgameTableInit(tBGameTable* gamesTable) { gamesTable->nBGames = 0; }
...
int bgameCmp(tBGame g1, tBGame g2)
{
    ...
    if(!different) {
        different = (g1.bgameId != g2.bgameId) || (g1.age != g2.age) || (g1.duration != g2.duration) ||
                    (g1.category != g2.category) || (g1.launchDate != g2.launchDate) || (g1.price != g2.price) ||
                    (g1.discount != g2.discount) || (g1.availability != g2.availability);
    }
    return different;
#else
    return true;
...
void bgameTableFilterByCategory(tBGameTable* tabBGames, tGameCategory category, tBGameTable* tabBGames2)
{
    ...
    tabBGames2->nBGames = 0;

    // For each game in the first table
    for(int i = 0; i < tabBGames->nBGames; i++) {

        // Check if it belongs to the indicated category
        if(tabBGames->table[i].category == category) {

            // Copy the game into the second table
            tabBGames2->table[tabBGames2->nBGames] = tabBGames->table[i];
            tabBGames2->nBGames++;
        }
    }
