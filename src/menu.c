    ...
    if(tabStores.nStores == 0) {
        ...
        for(i = 0; i < tabStores.nStores; i++) {
            printf("%d; %s; %s; %d; %.2f\n", (int)tabStores.table[i].storeId, tabStores.table[i].name,
                   tabStores.table[i].city, tabStores.table[i].maxStorage, tabStores.table[i].specialDiscount);
        }
    }
}
        ...
        for(i = 0; i < tabStores.nStores; i++) {
            ...
            if(store.inventory.nStoredBGames == 0) {
                ...
                for(j = 0; j < store.inventory.nBGames; j++) {
                    printf("%d; %s; %d; %d; %d; %d; %.2f; %.2f; %d\n", (int)bgames.table[j].bgameId,
                           bgames.table[j].bgName, bgames.table[j].age, bgames.table[j].duration,
                           (int)bgames.table[j].category, bgames.table[j].launchDate, bgames.table[j].price,
                           bgames.table[j].discount, bgames.table[j].availability);
                }
#endif
            }
        }
...
void purchaseMenu(tAppData* appData)
{
    ...
    while(option != PURCHASE_MENU_EXIT) {
        ...
        if(option == PURCHASE_MENU_LIST) {
            ...
            purchasesTable = appData->purchases;
            printPurchaseTickets(&storesTable, &purchasesTable);
        }
        /* Show list of options and ask the user for an option */
        printPurchasesMenuOptions();
        option = getPurchasesMenuOption();
...
void printPurchasesTable(tPurchaseTable* tabPurchases)
    ...
    if(tabPurchases->nPurchases == 0) {
        ...
        for(i = 0; i < tabPurchases->nPurchases; i++) {
            printf("\nPurchase id %d; Store id %d; nOrders %d; %d/%d/%d; %.2f\n",
                   (int)tabPurchases->table[i].purchaseId, (int)tabPurchases->table[i].storeId,
                   tabPurchases->table[i].nOrders, tabPurchases->table[i].date.day, tabPurchases->table[i].date.month,
                   tabPurchases->table[i].date.year, tabPurchases->table[i].price);
            printf("ORDERS:\n");
            for(j = 0; j < tabPurchases->table[i].nOrders; j++) {
                printf("%s %d %.2f\n", tabPurchases->table[i].orders[j].bgame,
                       tabPurchases->table[i].orders[j].nPurchased, tabPurchases->table[i].orders[j].price);
            }
        }
    }
}
